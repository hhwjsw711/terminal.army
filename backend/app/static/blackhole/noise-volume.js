/* Deterministic tiled value-noise lattice shared by the disk shader and the
 * headless still renderer.
 *
 * Vendored from vercel-labs/vgpu, apps/docs/examples/optimized-black-hole/
 * noise-volume.mjs, commit f495c9948e9b5109aa486c6754ef7c1a451573f5
 * (2026-08-26). MIT License, Copyright (c) 2025 Vercel, Inc. Full text in
 * scripts/blackhole/upstream/LICENSE.vercel.
 *
 * Changed from upstream: renamed to .js so the browser and Node load the same
 * file, and the JSDoc type imports of "vgpu" were dropped because nothing here
 * resolves bare specifiers. The lattice itself is byte for byte the original,
 * which is what keeps the still image and the live scene identical.
 */
...
/**
 * @param {object} gpu the vgpu device handle
 * @param {number} [size]
 * @param {string} [label]
 */
...
/** @param {object} vgpu the vgpu module namespace @param {object} gpu the vgpu device handle */
